...
def _make_autocast_ctx(dtype_str: str):
    ...
    `dtype_str` in {"fp32", "bf16", "fp16"}; fp32 returns a nullcontext (no autocast).
    Wrap model.sample(...) calls with this so per-step diffusion forwards run in
    half precision while weights stay in fp32.
    """
    ...
    raise ValueError(f"Unknown inference_dtype={dtype_str!r}; expected fp32 / bf16 / fp16.")
...
def train(cfg: DictConfig):
    ...
    inference_dtype = str(cfg.get("inference_dtype", "fp32"))
    print(f"inference_dtype: {inference_dtype}")
    ...
    model = SplatBeliefState(
        ...
        obj_permanence_observed_mode=str(cfg.get("obj_permanence_observed_mode", "live")),
    ...
    print(
        "[temporal_inference] object permanence: "
        f"mode={model.obj_permanence_mode}, "
        f"observed_mode={model.obj_permanence_observed_mode}, "
    ...
    if cfg.get("inference_indices") is not None:
        interesting_indices = [
            tuple(int(value) for value in item)
            for item in cfg.inference_indices
        ]
    elif cfg.inference_sample_from_dataset:
        ...
        for idx, i in enumerate(interesting_indices):
            ...
            for state_t, update_t in enumerate(key_frame_indices):
                ...
                for k, v in list(inp.items()):
                    if k != "num_frames_render" and torch.is_tensor(v):
                        inp[k] = v.unsqueeze(0)
                
                inp["num_frames_render"] = num_frames

                with _make_autocast_ctx(inference_dtype):
                ...
                if not state_t==len(key_frame_indices)-1:
                    ...
                    # Autoregressive imagination: keep each imagined keyframe's
                    # belief (accumulate) instead of re-encoding it into observed
                    # history — re-encoding loses joint consistency with the
                    # observed frames and seams the observed regions.
                    imagine_model.model.imagine_mode = True
                    ...
                    for imagine_t in range(state_t+1, len(key_frame_indices)):
                        ...
                        for k, v in list(imagine_input.items()):
                            if k != "num_frames_render" and torch.is_tensor(v):
                                imagine_input[k] = v.unsqueeze(0)
                        inp["num_frames_render"] = num_frames
                        # import ipdb; ipdb.set_trace()
                        with _make_autocast_ctx(inference_dtype):
