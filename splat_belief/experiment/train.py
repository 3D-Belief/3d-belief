...
def train(cfg: DictConfig):
    ...
    repa_encoder_weights = cfg.get("repa_encoder_weights", None)
    ...
    if cfg.model.encoder.backbone.use_repa:
        repa_encoder, _, _ = load_repa_encoder(
            ...
            weights_path=repa_encoder_weights,
        )
    ...
    if use_semantic and use_reg_model:
        reg_model_weights = cfg.model.encoder.get("reg_model_weights", None)
        dino_model = build_2d_model(model_name=reg_model_name, weights_path=reg_model_weights)

    # Optional class-text embedding table for the open-vocab semantic head.
    ...
    if use_semantic and semantic_supervision_mode in ("class_text_only", "class_text_image"):
        ...
        class_text_table = payload["embeddings"]  # (N+1, D)
    ...
    trainer = Trainer(
        ...
        finetune_component=cfg.finetune_component,
