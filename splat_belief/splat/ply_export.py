...
def compute_ply_normalization(means: Float[Tensor, "gaussian 3"]):
    """Center/scale used by export_ply. Pass the result to every export that
    must share one coordinate frame (e.g. observed + imagined splits of a scene)."""
    center = means.median(dim=0).values
    scale_factor = (means - center).abs().quantile(0.95, dim=0).max().clamp_min(1e-8)
...
def export_ply(
    ...
    path: Path,
    normalization: tuple[Tensor, Tensor] | None = None,
):
    # Shift the scene so the median Gaussian is at the origin, then rescale so
    # most Gaussians fall in [-1, 1].  When `normalization` is given, reuse that
    # (center, scale) so split exports share one coordinate frame.
    if normalization is None:
        center, scale_factor = compute_ply_normalization(means)
    else:
        center, scale_factor = normalization
        center = center.to(device=means.device, dtype=means.dtype)
        scale_factor = scale_factor.to(device=means.device, dtype=means.dtype).clamp_min(1e-8)
    means = means - center
    ...
    # Guard against empty input (e.g. a frustum-masked imagined.ply that
    # ended up with zero Gaussians) — scipy.Rotation.from_quat([]) crashes
    # with "Invalid shape in axis 0: 0".
