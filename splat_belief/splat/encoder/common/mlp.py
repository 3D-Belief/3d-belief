# Adapted from https://github.com/NVlabs/Sana/blob/main/diffusion/model/nets/basic_modules.py
...
def val2list(x, repeat_time=1) -> list:  # type: ignore
    """Repeat `val` for `repeat_time` times and return the list or val if list/tuple."""
    if isinstance(x, (list, tuple)):
        return list(x)
    return [x for _ in range(repeat_time)]

def val2tuple(x, min_len: int = 1, idx_repeat: int = -1) -> tuple:  # type: ignore
