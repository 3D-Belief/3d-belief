...
@dataclass
class EncoderVideoDiffusionCfg:
    name: Literal["video_diff"]
    d_feature: int
...
class EncoderVideoDiffusion(Encoder[EncoderVideoDiffusionCfg]):
    backbone: Backbone
    ...
    def __init__(self, cfg: EncoderVideoDiffusionCfg) -> None:
        super().__init__(cfg)
    ...
    def forward(
        ...
        features = self.backbone(image, t, external_cond=external_cond) # (B, V, C, H, W)
        if isinstance(features, tuple):
            features, repa_pred = features
