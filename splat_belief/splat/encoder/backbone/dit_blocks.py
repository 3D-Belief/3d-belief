...
from ..common.embeddings import RotaryEmbeddingND
from ..common.prope import prope_dot_product_attention
        ...
        if self.fused_attn:
            ...
            if self.prope_attn:
                x = prope_dot_product_attention(
                    ...
                    patches_y=patches_y,
                    image_width=1,
                    image_height=1,  # intrinsics already normalized
                )
            else:
...
class DiTBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs: dict,
        ...
        self.attn = Attention(
            hidden_size, num_heads=num_heads, qkv_bias=True, rope=rope, prope_attn=prope_attn, **block_kwargs
        )
    ...
    def forward(
        ...
        Args:
            x: Input tensor of shape (B, N, C).
            c: Conditioning tensor of shape (B, N, C).
            viewmats: Optional tensor of shape (B, 4, 4) representing the camera extrinsics.
            Ks: Optional tensor of shape (B, 3, 3) representing the camera intrinsics.
            patches_x: Optional float representing the patch size in x direction (in pixels).
            patches_y: Optional float representing the patch size in y direction (in pixels).
        """
