...
from ..common.embeddings import RotaryEmbedding3D, StochasticTimeEmbedding, RandomDropoutCondEmbedding
from .dit_base import SinusoidalPositionalEmbedding
...
class UViT3D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # ------------------------------- Configuration --------------------------------
        # these config closely follow the notation in the SiD2 paper
        ...
        self.use_vggt_alignment = cfg.use_vggt_alignment
        ...
        if self.use_repa:
            ...
            self.repa_size = cfg.repa_size
            self.repa_mlp = nn.Sequential(
                nn.Linear(self.emb_channels, 2 * self.emb_channels),
                nn.SiLU(),
                nn.Linear(2 * self.emb_channels, 2 * self.emb_channels),
                nn.SiLU(),
                nn.Linear(2 * self.emb_channels, self.repa_z_dim),
            )
