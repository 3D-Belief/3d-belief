...
from ..common.embeddings import RotaryEmbedding3D
...
class DiTBase(nn.Module):
    ...
    def forward(
        ...
        """
        viewmats = viewmats if viewmats is not None else None
        Ks = Ks if Ks is not None else None
        patches_x = patches_x if patches_x is not None else None
        patches_y = patches_y if patches_y is not None else None
        ...
        for i, (block, temporal_block) in enumerate(
            ...
            if viewmats is not None:
                execute_in_parallel(
                    lambda x, c, batch_size: self.checkpoint(
                        block, x, c, viewmats, Ks, patches_x, patches_y
                    )
                )
            else:
                execute_in_parallel(
                    lambda x, c, batch_size: self.checkpoint(block, x, c)
                )
