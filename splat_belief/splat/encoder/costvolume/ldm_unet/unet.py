...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, n_frames=None):
        params = (x,) if n_frames is None else (x, n_frames)
        should_checkpoint = (
            self.use_checkpoint
            and any(p.requires_grad for p in self.parameters())
            and (isinstance(x, torch.Tensor) and x.requires_grad)
        )

        if should_checkpoint:
            return checkpoint(self._forward, params, self.parameters(), True)
        else:
            return self._forward(*params)

    def _forward(self, x, n_frames=None):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)

        if self.postnorm:
            qkv = self.qkv(x)
            h = self.attention(qkv, n_frames)
            h = self.proj_out(h)
            h = self.norm(h)
        else:
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv, n_frames=None):
        """
        Apply QKV attention.
        ...
        n_v = n_frames if n_frames is not None else self.n_frames
        if self.use_cross_view_self_attn:
            qkv = rearrange(qkv, "(v b) n t -> b n (v t)", v=n_v)
        ...
        if self.use_cross_view_self_attn:
            a = rearrange(a, "b n (v t) -> (v b) n t", v=n_v)
