...
def warp_with_pose_depth_candidates(
    ...
    # sample features
    # a workaround to avoid CUDNN_STATUS_NOT_SUPPORTED
...
def prepare_feat_proj_data_lists(
    ...
    if nn_matrix is not None:
        nn_v = nn_matrix.shape[-1]
        assert nn_v <= v, "nearest n views should be smaller than v"
        assert not legacy_2views, "does not support legacy code for nearest view config"
        assert v > 2, "must have at least 3 context views for nn selection"
        cameras_dist_index = nn_matrix
        # # calculate nearest views based on camera location
        # with torch.no_grad():
        ...
        for idx in range(1, nn_v):
            cur_view_order = cameras_dist_index[:, :, idx]
            cur_feat = torch.gather(
                features,
                dim=1,
                index=repeat(cur_view_order, "b v -> b v c h w", c=c, h=h, w=w),
            )
            feat_lists.append(
                rearrange(cur_feat, "b v ... -> (v b) ...")
            )  # (vxb c h w)
            cur_view_extr = torch.gather(
                extrinsics, dim=1, index=repeat(cur_view_order, "b v -> b v 4 4")
            )
            cur_ref_pose_to_v0s = cur_view_extr.inverse() @ extrinsics
            pose_curr_lists.append(
                rearrange(cur_ref_pose_to_v0s, "b v ... -> (v b) ...")
            )
    else:
        for idx in range(1, v):
            ...
            if not (v == 2 and legacy_2views):
                ...
                cur_ref_pose_to_v0s = torch.cat(
                    cur_ref_pose_to_v0_list, dim=0
                )  # (vxb c h w)
...
class DepthPredictorMultiView(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.legacy_2views = legacy_2views
    ...
    def forward(
        ...
        if self.use_time_embedder:
            ...
            features = features + time_emb_full
            features = rearrange(features, "b v (h w) c -> b v c h w", h=h, w=w)
        ...
        feat_comb_lists, intr_curr, pose_curr_lists, disp_candi_curr = (
            prepare_feat_proj_data_lists(
                ...
                legacy_2views=self.legacy_2views,
                nn_matrix=nn_matrix,
            )
        )
        ...
        if cnn_features is None:
            feat_cat = torch.cat((feat01, feat01), dim=1)
        else:
            feat_cat = torch.cat((feat01, cnn_features), dim=1)
        proj_feat_in_fullres = self.upsampler(feat_cat)
