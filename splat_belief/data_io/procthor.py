...
class ProcTHORDataset(Dataset):
    ...
    def __init__(
        ...
        for scene_path in scene_path_list:
            ...
            seg_file = scene_path / "semantic_trajectory.mp4"
            
            if rgb_file.exists() and depth_file.exists() and pose_file.exists() and seg_file.exists():
    ...
    def read_frame(self, scene_path, frame_id):
        ...
        Returns:
            ...
            seg_color: Segmentation color tensor [3, H, W] in [0, 1]
        ...
        # Read segmentation from video (nearest interpolation to preserve instance colors)
        seg_file = scene_path / "semantic_trajectory.mp4"
        ...
        return rgb, depth, depth_mask, seg_color, cam_param
    ...
    def __getitem__(self, index: int):
        ...
        trgt_intrinsics = []
        for id in trgt_idx:
            rgb, depth, depth_mask, seg_color, cam_param = self.read_frame(scene_path, id)
            trgt_rgbs.append(rgb)
            trgt_depths.append(depth)
            trgt_depth_masks.append(depth_mask)
            trgt_segs.append(seg_color)
        ...
        trgt_seg = torch.stack(trgt_segs, axis=0)
        ...
        ctxt_seg = torch.stack(ctxt_segs, axis=0)
        ...
        if self.intermediate and (intm_idx is not None):
            ...
            intm_seg = torch.stack(intm_segs, axis=0)
        ...
        ret_dict = {
                ...
                "ctxt_seg_color": ctxt_seg,
                "trgt_seg_color": trgt_seg,
            }
    ...
    def data_for_video(self, video_idx, ctxt_idx, trgt_idx, num_frames_render=20):
        ...
        for i in range(1, num_frames_render + 1):
            if noflip:
                ...
                id = trgt_idx[0] + i
            _, _, _, _, cam_param = self.read_frame(scene_path, id)
    ...
    def data_for_temporal(self, video_idx: int, frames_render: Union[int, List]=20):
        ...
        ret = {
                ...
                "seg_colors": [torch.stack([seg], axis=0) for seg in seg_colors],
