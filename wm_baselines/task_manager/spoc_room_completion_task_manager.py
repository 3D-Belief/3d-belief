...
class SpocRoomCompletionTaskManager(BaseTaskManager):
    def __init__(self, embodied_config: DictConfig, stretch_controller: StretchController, **kwargs):
        ...
        self.imagination_object_lists = None
        self.done = False
    ...
    def current_ep_name(self):
        return f"{self.current_episode_index}_{self.episodes[self.current_episode_index]['house_index']}_{self.episodes[self.current_episode_index]['destination_room']}"
    ...
    def get_observation(self):
        ...
        self.imagination_object_lists = [self.observations[self._current_step + idx]['objects'] for idx in imagination_key_indices]
        self._current_step += 1
        assert self._current_step <= 1
        self.done = True
        return obs
    ...
    def reset(self, idx=None):
        ...
        self.current_destination_room = episode["destination_room"]
        ...
        self.imagination_object_lists = []
        self._current_step = 0
        self._load_observation()

        print(f"Starting episode {self.current_episode_index}: House {self.current_house_index}, Destination Room: {self.current_destination_room}")
    ...
    def get_final_log(self, metrics: dict = None):
        ...
        final_log = {
            ...
            "destination_room": self.current_destination_room,
    ...
    def _load_episode(self, episode_path: Path):
        ...
        episode = {
            ...
            "destination_room": metadata["dest_room_label"],
        }
        return episode
    ...
    def _load_observation(self):
        ...
        depth_files = sorted(list(depth_path.glob("*.npy")))
        # load all depth images using npy
        depths = [np.load(str(f)) for f in depth_files]
        ...
        semantic_metas = []
        for f in semantic_meta_files:
            with open(f, "r") as jf:
                meta = json.load(jf)["scene_metadata"]["object_categories"]
                assert isinstance(meta, list)
                meta = [self.lower_object_name(m) for m in meta if m not in ["HousePlant", "Unknown"]]
                semantic_metas.append(meta)
        # Load Poses
    ...
    def _calculate_gt_metrics(self) -> Dict[str, Any]:
        ...
        # ## DEBUG
        # obs_occupancy.save_occupancy_map("/home/ubuntu/jianwen-us-midwest-1/shulab-jhu/codebase/embodied_tasks/belief_baselines/outputs/debug/obs_occupancy.png")
        ret = {
    ...
    def _calculate_rendered_metrics(self, assets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        object_list = self.imagination_object_lists[-1]
        response = self.vlm.prompt_furniture_in_image(rgb, object_list)
        recognized_objects = response["parsed"]["furniture"]
        present_objects = response["parsed"]["present"]
        extra_objects = response["parsed"]["extra"]
        TP = len(present_objects)
        FP = len(extra_objects)
        FN = len(recognized_objects) - TP
        precision = TP / (TP + FP) if (TP + FP) > 0 else 0.0
        recall = TP / (TP + FN) if (TP + FN) > 0 else 0.0
        f1_score = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
        
        # ## DEBUG
        # belief_occupancy.save_occupancy_map("/home/ubuntu/jianwen-us-midwest-1/shulab-jhu/codebase/embodied_tasks/belief_baselines/outputs/debug/belief_occupancy.png")
        ret = {
            "belief_occupancy": belief_occupancy,
            "recognized_objects": recognized_objects,
            "present_objects": present_objects,
            "extra_objects": extra_objects,
            "object_coverage": len(present_objects) / len(recognized_objects) if len(recognized_objects) > 0 else 0.0,
            "precision": precision,
            "recall": recall,
            "f1_score": f1_score,
        }
