...
def sample_frontier_goals_balanced_turns(
    ...
    *,
    ...
    sectors w.r.t. the robot's heading.
    ...
    # Collect ALL candidates that pass everything except the max-distance upper bound
    goals: List[np.ndarray] = []
    fwds: List[np.ndarray] = []
    bearings: List[float] = []
    ...
    for r, c in cand_rc:
        ...
        # relative bearing (robot -> goal) vs robot heading, normalized to [-π, π]
        bx, bz = (x - rx), (z - rz)
        bn = math.hypot(bx, bz)
        if bn < 1e-6:
            continue
        ...
        # keep only within requested span (e.g., ±90°)
        if abs(theta) > span:
            continue
    ...
    if not strict_idx:
        if fallback_max_goal_dist_m is None:
            candidate_idx = list(range(len(goals)))  # no upper bound
        else:
    ...
    # Optional NMS / Poisson-disc to remove spatial near-duplicates (on candidate subset)
    kept_idx = candidate_idx
    if nms_radius_m > 0:
        rad_cells = max(1, int(round(nms_radius_m / res)))
        ...
        order = rng.permutation(len(kept_idx))
    ...
    # ---- Balance across LEFT / FORWARD / RIGHT bins ----
    ...
    rng.shuffle(left_idxs)
    rng.shuffle(fwd_idxs)
    rng.shuffle(right_idxs)

    base = k // 3
    take_left  = min(base, len(left_idxs))
    take_fwd   = min(base, len(fwd_idxs))
    take_right = min(base, len(right_idxs))
    ...
    leftovers = []
    if len(left_idxs) > take_left:
        leftovers.extend(left_idxs[take_left:])
    if len(fwd_idxs) > take_fwd:
        leftovers.extend(fwd_idxs[take_fwd:])
    if len(right_idxs) > take_right:
    ...
    # optional interleaving L-F-R (round-robin)
