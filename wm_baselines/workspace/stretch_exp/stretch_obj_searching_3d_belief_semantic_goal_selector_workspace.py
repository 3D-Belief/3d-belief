...
class StretchObjSearching3DBeliefSemanticGoalSelectorWorkspace(BaseWorkspace):
    def __init__(self, config: DictConfig):
        # instantiate stretch controller
        robot_ip = config.get("robot_ip", "192.168.1.10")
        stretch_controller = HomeRobotZmqClient(robot_ip=robot_ip)
        ...
        for ep_idx, ep in enumerate(self.task_manager.episodes):
            try:
                ...
                while (step < timeout) and (not done):
                    ...
                    done = self.task_manager.is_done(self.agent.current_observation['rgb'])
...
def main(cfg: DictConfig):
    workspace = StretchObjSearching3DBeliefSemanticGoalSelectorWorkspace(cfg)
    workspace.run()
