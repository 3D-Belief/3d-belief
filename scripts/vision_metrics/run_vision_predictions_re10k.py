...
DEFAULT_RE10K_ROOT = Path("data/re10k")
DEFAULT_BELIEF_CKPT = Path("checkpoints/3d_belief_re10k_256.pt")
DEFAULT_DFOT_CKPT = Path("checkpoints/DFoT_RE10K.ckpt")
DEFAULT_DFOT_REPO = Path("third_party/dfot")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gen3c-python",
        default=os.environ.get("GEN3C_PYTHON", "python"),
    )
    parser.add_argument("--gen3c-repo", type=Path, default=Path("third_party/gen3c"))
    parser.add_argument("--gen3c-checkpoint-dir", type=Path, default=None)
    parser.add_argument(
        "--gen3c-cuda-home",
        type=Path,
        default=Path(os.environ["GEN3C_CUDA_HOME"]) if "GEN3C_CUDA_HOME" in os.environ else None,
    )
