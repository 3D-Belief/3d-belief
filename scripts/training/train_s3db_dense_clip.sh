#!/bin/bash
# Training: s3db_dataset, Dense Layout + SG + Recon + FiLM + Dense CLIP Layout Loss
#
# Key differences from train_s3db_film_clip.sh:
#   - use_semantic=true  (enables per-Gaussian feature splatting + to_semantic MLP)
#   - dense_clip_layout_loss_weight=1.0  (new dense per-pixel CLIP text embedding supervision)
set -e
...
torchrun --nnodes 1 --nproc_per_node 1 --master_port $MASTER_PORT \
    ...
    stage=train \
    results_folder=outputs/training/s3db_dense_clip \
