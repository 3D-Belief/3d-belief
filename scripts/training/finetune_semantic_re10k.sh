...
# RE10K semantic-head fine-tune. RE10K is real-world video with no GT depth, so depth
# supervision and the depth mask are off and the context window is larger than SPOC.
# Override the dataset root / base checkpoint via env vars if they live elsewhere.
...
CUDA_LAUNCH_BLOCKING=1 torchrun --nnodes 1 --nproc_per_node $NGPUS --master_port $MASTER_PORT\
    ...
    results_folder=outputs/training/re10k_semantic \
