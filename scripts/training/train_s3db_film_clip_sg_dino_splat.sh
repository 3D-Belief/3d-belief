#!/bin/bash
# Training: s3db_dataset, Dense Layout + SG + Recon + FiLM + Splatted DINOv2 Loss
#
# Key difference from train_s3db_film_clip_sg.sh:
#   - Enables use_dino_splat=true: projects splatted features to DINOv2 space
#   - Uses dino_splat_loss_weight=1.0 for direct feature supervision
#   - render_features=true required for Gaussian feature splatting
set -e
...
torchrun --nnodes 1 --nproc_per_node 1 --master_port $MASTER_PORT \
    ...
    model.encoder.use_dino_splat=true \
    model.encoder.d_dino_splat=768 \
    ...
    model.encoder.render_features=true \
    ...
    model.encoder.backbone.layout_recon_mode=open_vocab \
    ...
    name=s3db_film_clip_sg_dino_splat \
    wandb=local \
    clean_target=false \
    use_identity=true \
    intermediate=true \
