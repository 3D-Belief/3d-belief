...
class SPOCDataset(Dataset):
    ...
    def __init__(
        ...
        for i, scene_path in enumerate(scene_path_list):
            ...
            timestamps = [int(pose_file.name.split(".")[0].split("_")[-2]) for pose_file in pose_files]
            sorted_ids = np.argsort(timestamps)
            all_rgb_files.append(np.array(rgb_files))
            all_depth_files.append(np.array(depth_files))
    ...
    def read_frame(self, rgb_files, depth_files, pose_files, id):
        video_file = rgb_files[0]
        # read rgb video frame
        ...
        video_file = depth_files[0]
        # read depth video frame
        cap = cv2.VideoCapture(str(video_file))
        cap.set(cv2.CAP_PROP_POS_FRAMES, id)
        ret, frame = cap.read()
        # apply depth scale
        frame = frame.astype(np.float32) / 1000.0  # depth is stored in mm
        cap.release()
        depth = torch.tensor(frame, dtype=torch.float32).unsqueeze(0)
        depth = depth[..., 0]
    ...
    def __getitem__(self, index: int):
        ...
        start_idx = self.rng.choice(max(len(pose_files)-self.context_max_distance, 2), 1)[0]

        if start_idx+1>=len(pose_files)-1:
            return fallback()
    ...
    def data_for_video(self, video_idx, ctxt_idx, trgt_idx, num_frames_render=20):
        ...
        render_poses = []
        num_frames_render = min(ctxt_idx[0], len(pose_files) - 1) - min(
            trgt_idx[0], len(pose_files) - 1
        )
        noflip = False
        ...
        for i in range(1, num_frames_render + 1):
            ...
            _, _, _, cam_param = self.read_frame(rgb_files, depth_files, pose_files, id)
            render_poses.append(cam_param.c2w_mat)
        render_poses = torch.tensor(np.array(render_poses)).float()

        print(
            f"ctxt_idx: {ctxt_idx}, trgt_idx: {trgt_idx}, num_frames_render: {num_frames_render}, len(pose_files): {len(pose_files)}"
        )
    ...
    def data_for_temporal(self, video_idx: int, frames_render: Union[int, List]=20):
        ...
        if isinstance(frames_render, (int, np.integer)):
            ids = select_random_sequence(self.rng, len(pose_files), frames_render)
            start_idx = ids[0]
            end_idx = ids[-1]
        else:
            start_idx = frames_render[0]
            end_idx = frames_render[1]
            assert start_idx<len(pose_files) and end_idx<len(pose_files)
