...
@dataclass
class EncoderUViTMVSplatCfg:
    name: Literal["uvitmvsplat"]
    d_feature: int
...
class EncoderUViTMVSplat(Encoder[EncoderUViTMVSplatCfg]):
    ...
    def __init__(self, cfg: EncoderUViTMVSplatCfg) -> None:
        super().__init__(cfg)
    ...
    def init_from_ckpt(self, path: str) -> None:
        ...
        if path.endswith(".ckpt"):
            ...
            enc_dict = torch.load(path, map_location="cpu")
        else:
            raise NotImplementedError(f"Unknown checkpoint type: {path}")

        # sometimes nested under 'state_dict'
        if "state_dict" in enc_dict:
            enc_dict = enc_dict["state_dict"]
        
        if "model" in enc_dict:
            enc_dict = enc_dict["model"]
        ...
        if path.endswith(".pt"):
            enc_dict = OrderedDict(
                {k[14:]: v for k, v in enc_dict.items() if k.startswith("model.encoder")}
            )
        ...
        # handle shape mismatch if cfg.fit_ckpt is True
        if self.cfg.fit_ckpt:
            print("Fitting pretrained encoder weights to new model...")
